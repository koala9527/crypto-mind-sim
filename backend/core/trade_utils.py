...
from backend.core.models import TradeSide
...
def calculate_fee(price: float, quantity: float) -> float:
    return round(calculate_notional_value(price, quantity) * settings.TRADING_FEE_RATE, 8)
...
def calculate_liquidation_price(entry_price: float, leverage: int, side: TradeSide) -> float | None:
    if not entry_price or leverage <= 0:
        return None

    move_ratio = settings.LIQUIDATION_THRESHOLD / (leverage ** 2)
